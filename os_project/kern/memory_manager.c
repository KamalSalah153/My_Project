...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int AllFrames=(size+PAGE_SIZE-1)/PAGE_SIZE;
	uint32 va_loop=virtual_address;
	uint32*pg_table=NULL;
	for (int i=0;i<AllFrames;i++){
		pf_add_empty_env_page(e,va_loop,0);
		va_loop+=PAGE_SIZE;
		//(e->disk_env_pgdir)[PTX(va_loop)]|=PERM_WRITEABLE;
	//cprintf("HELLO WORLD\n");
	}
...
{
	uint32 num=(size+PAGE_SIZE-1)/PAGE_SIZE;
	uint32 va=virtual_address;
	// Remove pages on working set
	struct Frame_Info* ptr_frame_info = NULL;
	uint32* ptr_page_table;
	uint32 table_va=virtual_address;
	for(int i=0; i<e->page_WS_max_size; i++)
	{
		uint32 address_of_page_WS =env_page_ws_get_virtual_address(e,i);
		address_of_page_WS=ROUNDDOWN(address_of_page_WS,PAGE_SIZE);
		if((address_of_page_WS>=va) && (address_of_page_WS<(va+(num*PAGE_SIZE))))
		{
			uint32* ptr_page_table=NULL;
			get_page_table(e->env_page_directory,(void*)address_of_page_WS,&ptr_page_table);
			unmap_frame(e->env_page_directory,(void*)address_of_page_WS);
			//env_page_ws_clear_entry(e,i);
			struct WorkingSetElement *ele;
			LIST_FOREACH(ele,&(curenv->ActiveList)){
				if (ele->virtual_address==address_of_page_WS){
					struct WorkingSetElement *tail_element_in_sec =  ele;
					LIST_REMOVE(&(curenv->ActiveList),ele);
			        pt_set_page_permissions(curenv,ele->virtual_address, 0,PERM_MODIFIED|PERM_PRESENT);
					ele->empty=1;
			        LIST_INSERT_HEAD(&curenv->PageWorkingSetList , ele);
					// 1 3 4
					// 5 6
					if (LIST_SIZE(&curenv->SecondList)>0){
						LIST_REMOVE(&(curenv->SecondList),curenv->SecondList.lh_first);
						pt_set_page_permissions(curenv,curenv->SecondList.lh_first->virtual_address, PERM_PRESENT,0);
						LIST_INSERT_TAIL(&(curenv->ActiveList),curenv->SecondList.lh_first);
						break;
					}
				}
			}
			LIST_FOREACH(ele,&(curenv->SecondList)){
				if (ele->virtual_address==address_of_page_WS){
					struct WorkingSetElement *tail_element_in_sec =  ele;
					LIST_REMOVE(&(curenv->SecondList),ele);
			        pt_set_page_permissions(curenv,tail_element_in_sec->virtual_address, 0,PERM_MODIFIED|PERM_PRESENT);
			...
			if (TableIsEmpty){
				uint32 table_pa=e->env_page_directory[PDX(address_of_page_WS)];
				table_pa>>=12;
				table_pa*=PAGE_SIZE;
				//cprintf("Physical Address: %x\n",table_pa);
				struct Frame_Info*Table_Info=to_frame_info(table_pa);
				Table_Info->references--;
				if (Table_Info->references==0)
					free_frame(Table_Info);
				uint32 Page_Table_Dir_entry=PDX(address_of_page_WS);
				e->env_page_directory[Page_Table_Dir_entry]=0;
			}
		}
	}
	//AllocatedFramesCount-=num;
	for(int i=0; i<num;i++){
		//cprintf("%x\n",va);
